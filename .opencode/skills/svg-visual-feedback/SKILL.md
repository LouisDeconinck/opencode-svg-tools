...
- `{ "operation": "clip-escape", "element": "blanket" }` — does the element poke outside the clip applied to it? Omit `element` to check every clipped element in the file.
- `{ "operation": "containment-check", "element": "artwork", "against_element": "silhouette" }` — is one element fully inside another's bounds?
...
- `"mode": "side-by-side"` — left and right next to each other (the default)
...
- `"mode": "difference"` — pixel diff: identical renders report 0 differing pixels, changed regions light up magenta and the differing area is reported in SVG coordinates
