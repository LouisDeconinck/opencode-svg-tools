...
- **`difference`** renders each file alone inside `left`'s coordinate space, then diffs the pixels: identical regions fade to pale gray, differing pixels turn magenta, and the output reports the differing pixel count plus the differing region in left-SVG coordinates. The comparison includes alpha (premultiplied RGBA), so transparency-only changes — e.g. a white shape appearing where nothing was — still count. `0 differing pixels` is a strong identical-render guarantee.
